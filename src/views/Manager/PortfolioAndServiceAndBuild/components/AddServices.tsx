...
import { createServices, getService, updateServices } from "@/services/settings-service";
...
interface DataServicesFormData{
    title: string,
    content: string
}

type FormErrors = {
  [K in keyof DataServicesFormData]?: string;
};

const AddServices: React.FC<AddServicesProps> = ({ onClose, onLoad, serviceId}) => {
    const [imageFile, setImageFile] = useState<File | null>(null);
    const notify = useNotification();
    const [formData, setFormData] = useState<DataServicesFormData>({
         title: '', content: ''
    })
    ...
    useEffect(() => {
        if(serviceId){
            const fetchService = async() => {
                ...
                const data = res.data as any as IServices;
                setFormData({
                    title: data.title,
    ...
    const handleChangeInput = (name: string, value: string | Dayjs | null | number) => {
        if(Object.prototype.hasOwnProperty.call(formData, name)){
            const validName = name as keyof DataServicesFormData;
    ...
    const handleSubmit = async(event: FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if(serviceId){
                //update
                res = await updateServices(serviceId, data);
