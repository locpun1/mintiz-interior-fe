...
import { forwardRef, Fragment, ReactNode, useEffect } from 'react';
...
export interface FormSelectProps<T extends string | number = number>
  extends Omit<SelectProps<T>, 'name' | 'onSelect' | 'value'> {
  name: string;
  options?: Option<T>[];
  ...
  return (
    <FormControl fullWidth error={Boolean(error)} disabled={disabled}>
      ...
      >
        ...
        {!options.length && <PlainMenuItem value={-1}>{t('no_option')}</PlainMenuItem>}
