...
const ProFormAutocomplete = <O extends FieldValues, V extends string | number>(
  ...
  const entries = useMemo(
    () =>
      options?.reduce<Map<string | number, Value<V>>>((acc, option, i) => {
  ...
  // Rollback
  useEffect(() => {
    if (!entries?.size || entries.has(value) || value === null || value === undefined) {
      return;
    }
    setValue(name, null);
  }, [value, entries, name]);
  ...
  return (
    <FormControl fullWidth error={Boolean(error)} disabled={disabled}>
      ...
      >
        <Autocomplete<V, false, false, false>
          id={name}
          disabled={disabled}
          multiple={false}
          {...(disabled && {
          ...
          noOptionsText={!options?.length && !actionText ? 'Không có kết quả' : actionText}
